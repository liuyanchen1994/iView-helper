...
export function encodeDocsUri(query?: Query): Uri {
  return Uri.parse(`iview-helper://search?${JSON.stringify(query)}`);
}
...
export class App {
  ...
  openDocs(query?: Query, title = 'iview-helper', editor = window.activeTextEditor){
  ...
  getWebviewContent(query: Query) {
    const config = workspace.getConfiguration('iview-helper');
